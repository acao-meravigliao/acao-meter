...
class ModBus
  ...
  class Resp < Msg
    ...
    def self.new_from_net(buf)
      raise Incomplete if buf.size < 5

      (address, function, payload_size) = buf.to_s.unpack('CCC')

      raise Incomplete if buf.size < (3+payload_size+2)
      #raise "Noise?" if buf.size > 3+payload_size+2

      data = buf.read(buf.size)

      # XXX TODO check address == address, function == function

      header = data.slice(0...3)
      payload = data.slice(3...(3+payload_size))
      (crc,) = data.slice((3+payload_size)...(3+payload_size+2)).unpack('n')
